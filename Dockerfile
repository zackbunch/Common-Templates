...
# Re-declare ARGs to make them available in the build stage
ARG CI_REGISTRY
ARG CI_JOB_TOKEN
ARG CI_API_V4_URL
ARG CI_PROJECT_ID
...
RUN apt-get update && apt-get install -y curl && \
    curl --header "JOB-TOKEN: $CI_JOB_TOKEN" "$CI_API_V4_URL/projects/$CI_PROJECT_ID/packages/generic/sqlcl/0.0.1/sqlcl.tar.gz" -o sqlcl.tar.gz && \
